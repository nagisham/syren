...
interface PipelineOptions<P extends any[], A = P, R = void> {
  request?: (...params: P) => A;
  response?: (args: A) => R;
  midleware?: (processor: Processor<A>, arg: A, api: PipelineApi) => void;
  ...
  return Object.assign(
    {
      ...
      midleware(processor: Processor<A>, arg: A, api: PipelineApi) {
        processor(arg, api);
      },
    },
    options,
...
export const pipeline: PipelineConstructor = <P extends any[], A = P, R = void>(
  ...
  const { processors, request, response, midleware } = pipeline_options(options);
